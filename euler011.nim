...
type
  Grid = array[gridSize * gridSize, Natural]
...
func initGrid(s: string): Grid =
  ...
  for line in s.strip.split('\n'):
    for num in line.strip.split():
      result[i] = num.parseInt
      inc i

proc `[]`(g: Grid, p: GridPosition): Natural =
  g[(p.row - 1) * gridSize + (p.col - 1)]
...
func product(g: Grid, start: GridPosition, d: Direction): Natural =
  var pos = start
  result = g[pos]
  for _ in 1 ..< productLength:
    pos = pos.nextNeighbor(d)
    result *= g[pos]
...
func largestProduct(g: Grid, d: Direction): Natural =
  ...
  for row in startRow..endRow:
    for col in startCol..endCol:
      let start = GridPosition(row: row, col: col)
      let candidate = g.product(start, d)
      if candidate > result:
        result = candidate

func largestProduct(g: Grid): Natural =
  result = 0
  for d in Direction.low..Direction.high:
    let candidate = g.largestProduct(d)
    if candidate > result:
      result = candidate
